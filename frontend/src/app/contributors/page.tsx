...
import { getRepoContributors, getRepos } from '@/lib/api';
...
export default async function ContributorsPage({
  ...
  const contributors = repoId
    ? await getRepoContributors(repoId).catch(() => [])
    : [];
  ...
  const repoSuffix = repoId ? `?repoId=${repoId}` : '';
      ...
      <div className="relative z-10 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-0.5">
            {contributors.length} contributors{repo ? ` — ${repo.fullName}` : ''}
          </p>
        </div>
      </div>

      {contributors.length === 0 ? (
        <div className="relative z-10 flex flex-col items-center justify-center py-24 text-zinc-500">
          <p className="text-sm">No contributors found. Select or add a repository.</p>
        </div>
      ) : (
                ...
                return (
                  <tr key={c.id} className="group hover:bg-zinc-800/30 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <Link
                        href={`/contributors/${c.id}${repoSuffix}`}
