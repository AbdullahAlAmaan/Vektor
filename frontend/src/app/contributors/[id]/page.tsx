  ...
  if (!contributor) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-zinc-500">
        ...
        <Link href={backHref} className="mt-3 text-sm text-blue-400 hover:text-blue-300">
        ...
        {recommendations.length > 0 ? (
          <div className="space-y-2">
            {recommendations.map((rec, i) => {
              const complexityHint = (rec as { complexityHint?: string }).complexityHint;
              return (
                <div
                  key={rec.issueId}
                ...
                >
                  ...
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <span className="text-xs text-zinc-600 font-mono truncate">{rec.issueId}</span>
                    {complexityHint && (
                      <span className={`inline-flex items-center rounded px-1.5 py-0.5 text-xs font-medium flex-shrink-0 ${COMPLEXITY_COLORS[complexityHint] ?? ''}`}>
                        {complexityHint}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex gap-4 text-xs">
                    ...
                    <div className="text-center">
                      <p className="text-zinc-600 mb-0.5">text</p>
                      <p className="text-zinc-300 font-mono">{rec.breakdown.textSimilarity.toFixed(2)}</p>
                    </div>
                    <div className="text-center">
                    ...
                    <div className="text-center">
                      <p className="text-zinc-600 mb-0.5">workload</p>
                      <p className="text-red-400 font-mono">-{(rec.breakdown.workloadPenalty * 100).toFixed(0)}%</p>
                    </div>
                  </div>
