import { getEvaluationResults } from '@/lib/api';
...
export default async function EvaluationPage() {
  const results = await getEvaluationResults().catch(() => FALLBACK);
  const data = results.length > 0 ? results : FALLBACK;
  const latest = data[0];
      ...
      <div className="relative z-10 rounded-xl border border-zinc-800 bg-zinc-900/60 p-6 backdrop-blur-sm">
        ...
        <dl className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {[
            ...
            { label: 'Repository', value: 'expressjs/express' },
          ].map(({ label, value }) => (
            <div key={label}>
