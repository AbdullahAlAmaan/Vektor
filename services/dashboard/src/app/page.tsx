...
import { getHealth, getRepoContributors, REPO_ID } from '@/lib/api';
...
export default async function OverviewPage() {
  const [health, contributors] = await Promise.all([
    ...
    getRepoContributors(REPO_ID).catch(() => []),
  ]);
  ...
  const kpis = [
    { title: 'Contributors', value: contributors.length, icon: <Users className="h-5 w-5 text-blue-400" />, subtitle: 'expressjs/express' },
    ...
    { title: 'Model Version', value: 1, icon: <Cpu className="h-5 w-5 text-emerald-400" />, prefix: 'v', subtitle: 'scoring formula active' },
  ];
    ...
    <div className="relative space-y-8">
      ...
      <div className="relative z-10">
        ...
        <p className="text-sm text-zinc-500 mt-1">
          GitHub intelligence platform — contributor expertise & issue recommendations
        </p>
      </div>
